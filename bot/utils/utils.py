...
async def sample_fn(c, m):
    ...
    try:
        ...
        await edit_message_text(m, text=f'🤓 Sample video generated successfully now uploading process going on.')
        ...
        traceback.print_exc()
        await edit_message_text(m, text='Sorry sample video generation failed due to some problems😥.....')
...
async def trim_fn(c, m):
    ...
    if media_msg.empty:
        await m.reply_text('HMM..stupid why did you delete this file😠, so i cannot help you 😒.', True)
        c.CURRENT_PROCESSES[chat_id] -= 1
        return
    ...
    try:
        ...
        if not sample_file.exists():
            await snt.edit_text('SORRY BROH☹️,TRIMMING FAILED DUE TO SOME PROBLEMS....')
        ...
        await snt.edit_text('😎 owo Video trimmed successfully now uploading to tg.')
